...
public class BatchDiscordLog {
    private final String NOW = LocalDateTime.now().format(LOG_TIME); // システム日付を取得
    private final Color COLOR = Color.GREEN;

    // #### テキストチャンネル ####
    public final String TEXT_CHANNEL_NAME = BATCH_TEXT_CHANNEL_NAME;
    public final List<String> TEXT_CHANNEL_ID_LIST = BATCH_TEXT_CHANNEL_ID_LIST;
    ...
    public void outPutLog(String PLUGIN_NAME, String TABLE_NAME, String MESSAGE, String QUERY, HashMap<String, String> PARAM){
        ...
        for (String textChannelID : TEXT_CHANNEL_ID_LIST) {
            ...
            textChannel.sendMessageEmbeds(embedBuilder.build()).queue(); // Batchログを出力
        }
        // #### スレッドチャンネル ####
        for (String threadChannelID : THREAD_CHANNEL_ID_LIST) {
            ...
            threadChannel.sendMessageEmbeds(embedBuilder.build()).queue(); // Batchログを出力
        }
    }
